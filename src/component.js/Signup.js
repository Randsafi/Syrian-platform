    ...
    <ThemeProvider Provider theme={appTheme}>
     ...
     <Container maxWidth="sm" sx={{marginBottom:'400px', gap: 2, my: 3  }}>
      ...
      <Box style={{ display:'flex',alignItems: 'center',justifyContent:'center',flexDirection:'row' , gap: 2, my: 3 }}>
       <Typography variant="h4" color='#1e2830ff' >هل لديك حساب على المنصة السورية؟</Typography> 
       <Link to={"/login"}>
      ...
      <Stack sx={{ display: 'flex' ,alignItems: 'center',justifyContent:'center',flexDirection:'row' }}>
        <Button variant='outlined'sx={{borderRadius:'30px' ,marginLeft:'10px'}} color="inherit" onClick={() => window.open('https://accounts.google.com/signup', '_blank')}><GoogleIcon/>انشاء عبر جوجل</Button>
        <Button variant='outlined'sx={{borderRadius:'30px'}} color="inherit" onClick={() => window.open('https://appleid.apple.com/sign-up', '_blank')}><AppleIcon/>انشاء عبر آبل</Button>
      </Stack>
      ...
      <Stack
       ...
       sx={{display:'flex',
          ...
          gap: 2, my: 3,
          direction:'rtl' }}>
         <Typography variant="h6" sx={{ mr: 3 ,marginLeft:'410px' }}> الاسم</Typography>
         <TextField
          required
          id="outlined-input"
          value={formData.name}
